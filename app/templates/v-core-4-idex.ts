...
export const template = (config: PrinterConfiguration, helper: KlipperConfigHelper) => {
	// Emit an optional beacon-specific block when the primary toolhead uses the `beacon` probe.
	const hasBeacon = config.toolheads?.some((th) => !th.toolNumber && th.probe?.id === 'beacon');
	const beaconBlock = hasBeacon
		? `
...
[bed_mesh]
mesh_min: 20,20
mesh_max: ${config.size.x - 20},${config.size.y - 20}

[beacon]
contact_mesh_min: 20, 20
contact_mesh_max: ${config.size.x - 20},${config.size.y - 20}
`
		: '';
...
### HOMING
#############################################################################################################
${helper.renderProbeIncludes()}${beaconBlock}
