...
class RatOS:
	...
	self.desc_RESET_DC_ENDSTOP_CONFIGURATION = "Resets the dc-endstop.cfg configuration file, allowing reconfiguration of dual carriage endstop settings using the CONFIGURE_DC_ENDSTOP macro."
	def cmd_RESET_DC_ENDSTOP_CONFIGURATION(self, gcmd):
		...
		config_path = os.path.join(config_dir, 'dc-endstop.cfg')
		content = 
	...
	self.desc_CONFIGURE_DC_ENDSTOP = "Updates the dc-endstop.cfg configuration file, configuring dual carriage endstop settings taking account of the last measured IDEX toolhead offsets (for example, from VAOC)."
	def cmd_CONFIGURE_DC_ENDSTOP(self, gcmd):		
		...
		stepper_x_position_max = x.get_float("position_max", default=None)
		stepper_x_position_endstop = x.get_float("position_endstop", default=None)
		dual_carriage_position_max = dc.get_float("position_max", default=None)
		dual_carriage_position_endstop = dc.get_float("position_endstop", default=None)
		...
		if not ('idex_xoffset' in svv and 'idex_offset' in svv and 'idex_xcontrolpoint' in svv and 'idex_ycontrolpoint' in svv):
			self.console_echo('Missing IDEX variables', 'error', 'One or more required IDEX variables are missing from ratos-variables.cfg.')
			return
		idex_offset = float(svv['idex_offset'])
		...
		self.console_echo(
			...
			'followed by pasting the output into printer.cfg, you MUST remove the configuration you pasted,
		...
		content = 
			...
			'\n' + \
			'[dual_carriage]\n' + \
			f'position_max: {dual_carriage_position_max + idex_offset:.3f}\n' + \
