...
describe('invokeAction', async () => {
	...
	state.gcodeInfo = new MutableGCodeInfo('PrusaSlicer', semver.coerce('2.8')!, GCodeFlavour.PrusaSlicer, new Date());
...
describe('satisfiesFilter', async () => {
	test('simple match', () => {
		const gcodeInfo = new MutableGCodeInfo('PrusaSlicer', semver.coerce('2.8')!, GCodeFlavour.PrusaSlicer, new Date());
	...
	test('version not-match 1', () => {
		const gcodeInfo = new MutableGCodeInfo('PrusaSlicer', semver.coerce('1.8')!, GCodeFlavour.PrusaSlicer, new Date());
		const match = GCodeProcessor['satisfiesFilter'](gcodeInfo, [GCodeFlavour.PrusaSlicer, '>2.1']);
		expect(match).toStrictEqual(false);
	});
