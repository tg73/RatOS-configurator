...
describe('GCodeInfo', async () => {
	test('PrusaSlicer 2.8.0', () => {
		...
		let parsed = GCodeInfo.tryParseHeader(header);
	...
	test('already processed', async () => {
		const header =
			...
			'; processed by RatOS 2.0.2-518-g4ffef464 on 2025-01-01 at 08:30:00\n';
		let parsed = GCodeInfo.tryParseHeader(header);
		...
		expect(parsed!.processedByRatOSTimestamp).toEqual(new Date('2025-01-01 08:30:00'));
	});
	...
	test('getProcessedByRatosHeader', async () => {
		let header = await GCodeInfo.getProcessedByRatosHeader();
